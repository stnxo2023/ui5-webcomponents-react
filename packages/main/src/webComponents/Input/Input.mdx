...
<DocsHeader of={ComponentStories} subComponents={['InputIcon']} />
...
### Input with InputIcon

The `InputIcon` component represents an interactive icon that can be placed inside an `Input` via the `icon` slot.
Unlike the standard `Icon`, it provides button-like behavior with hover, focus, and active states.

<Canvas of={ComponentStories.WithInputIcon} />

<Markdown>{SubcomponentsSection}</Markdown>

## InputIcon

<Description of={InputIcon} />
<ArgTypesWithNote metaOf={ComponentStories} hideHTMLPropsNote of={InputIcon} />
