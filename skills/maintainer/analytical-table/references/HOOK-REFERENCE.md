...
**`pluginName` is required.** `ensurePluginOrder` (vendored at `react-table/publicUtils.tsx:92-123`) throws if a plugin lacks it. Examples in source: `hooks/useDynamicColumnWidths.ts:489` ensures `useColumnResizing` runs first; `hooks/useSelectionChangeCallback.ts:34` ensures `useRowSelect` runs first. Future plugins can use `ensurePluginOrder` against any internal hook by name.
...
| Hook                                         | Registration Points    | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| -------------------------------------------- | ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **useColumnDragAndDrop** (useDragAndDrop.ts) | `hooks.getHeaderProps` | Column reorder via drag-and-drop. Uses `dispatch` for `COLUMN_DND_START`/`COLUMN_DND_END` + `setColumnOrder`. No global disable knob — only per-column `disableDragAndDrop` is honored. Both the drop-**target** exclusion (`isStickyTarget`, `useDragAndDrop.ts`) and the drag-**source** exclusion (`isDraggable`, `ColumnHeaderContainer.tsx`) read the live `stickyStartIndices` (the target resolves the column's index in `visibleColumns`) — so they stay in lockstep across runtime `toggleStickyColumn` and width auto-disable. |
...
## Plugin Hooks (8 exported from the `AnalyticalTableHooks` namespace)

User-facing plugins passed via `tableHooks`. Imported as `AnalyticalTableHooks.<name>` from `@ui5/webcomponents-react`. All eight live in `pluginHooks/` and are re-exported through the namespace.
...
| **useStickyColumns**             | `useStickyColumns(options?: { onStickyColumnsChange?, onAutoToggleSticky? })` | `hooks.stateReducers`, `hooks.visibleColumnsDeps`, `hooks.visibleColumns`, `hooks.useInstance`, `hooks.getHeaderProps`, `hooks.columnHeaderModalItems` | **Experimental** frozen-start columns (`pluginHooks/useStickyColumns.ts`). State-driven (`state.stickyColumns`); reorders sticky columns to front, sets `instance.stickyStartIndices`/`totalStickyStartWidth`, contributes the popover Freeze/Unfreeze item. `onStickyColumnsChange` fires on popover toggles only; `onAutoToggleSticky` fires on width-driven auto-disable/re-enable (`{ enabled, stickyColumns }`; config persists across auto-disable). See the Sticky-columns section in SKILL.md. |
...
`useOptions`, `columns`, `columnsDeps`, `allColumns`, `allColumnsDeps`, `visibleColumns`, `visibleColumnsDeps`, `headerGroups`, `headerGroupsDeps`, `materializedColumns` (declared but never invoked by vendored `useTable`), `materializedColumnsDeps` (same), `accessValue` (per-cell during row materialization), `stateReducers`, `useControlledState`, `columnHeaderModalItems` (reduce-style; lets a plugin contribute menu items to the column-header popover — `makeDefaultPluginHooks.ts:78`, reduced in `useStyling.ts`; a non-empty result also flips the `hasPopover` gate so the popover opens even without sort/filter/group)
